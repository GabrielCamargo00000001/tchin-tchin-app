...
| _(Duolingo v2)_ | **"Treine seu Paladar" reconstruído de verdade** (`screens-treino-paladar.jsx`, store `tc.treino.v2`): **trilha visual** (path com unidades/nós: concluído/atual/bloqueado), HUD (streak 🔥 / vidas ❤️ / XP / **meta diária** em anel), lição com **exercícios variados** (múltipla escolha, V/F, associar) + feedback instantâneo, **tela de conclusão** (XP/precisão/tempo, baú de recompensa variável, badges, combo "lição perfeita"), **Liga semanal** (`treino-liga`, leaderboard com zona promoção/rebaixamento), mascote "Tchin", streak freeze, copy de notificação estilo Duo. Padrões fundamentados em pesquisa Firecrawl do Duolingo (Octalysis, loss aversion, variable reward) + estudo de mercado (decisão de compra + anti-elitismo). 9 lições em 3 unidades. **Onboarding guiado** pelo mascote na 1ª vez (`onboarded` no store) + botão "?" pra rever. |
| _(Duolingo v3)_ | Refinado com **43 telas de referência do Duolingo** (pasta do usuário). **Onboarding conversacional DA FEATURE** (mascote + 1 pergunta/tela: nível, objetivo, meta diária, meta de ofensiva, cristais de boas-vindas) — o onboarding do **app** NÃO foi tocado. **6 tipos de exercício** (múltipla escolha, V/F, associar, completar-frase com chips, banco de palavras, digitar). **Trilha polida**: nós circulares 3D com ícone por tipo + **baús de bônus** por unidade. **Cristais** (moeda) no HUD + comprar congelamento de ofensiva. **Meta de ofensiva** (7/14/30/50 dias, commitment). Store migrou p/ `tc.treino.v3`. **"Entenda melhor"** = explicação inline em linguagem clara após cada exercício (abre sozinha no erro). **Micro-vídeo-aula** (placeholder/demo — vídeo real entra com assets) no card de conceito. **Quests maiores** (texto e botões). Onboarding da feature enxuto (nível reaproveitado do app). **Aprenda bebendo** (`treino-aprender`): porta de entrada contextual — **escanear a garrafa** (→ scanner) OU **buscar/escolher tema** → cai na **mesma** micro-lição (mesmo XP/streak; sem duplicar conteúdo). Card de entrada na home + link pra biblioteca "Aprenda". Une os 2 mundos (Aprenda + Treine) num motor só, 2 portas. |
